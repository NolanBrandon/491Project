...
class MealPlanEntry(models.Model):
    ...
    def clean(self):
        if (self.food and self.recipe) or (not self.food and not self.recipe):
            raise ValidationError("Entry must have either food OR recipe, but not both or neither.")

    def __str__(self):
        item = self.food.name if self.food else self.recipe.name
        return f"{self.meal_plan_day} - {self.meal_type}: {item}"